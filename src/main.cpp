...
static void fade_led(int ledIdx) {
  ...
  for (int b = 0; b <= 100; b += FADE_STEP) {
    if (k_msgq_peek(&state_msgq, &peek_msg) == 0 && peek_msg.alert_level > 0)
      return;
    if (bt_get_brightness() > 0)
      return;
    leds[ledIdx].set_brightness(b);
    k_msleep(FADE_MS);
  }
...
void imu_thread_func(void *arg1, void *arg2, void *arg3) {
  while (1) {
    ...
    // Step A: Wake the chip up
    board_imu.wake();

    // The IMU is set to 26Hz ODR (Output Data Rate) in prj.conf.
    // 1 / 26Hz = ~38.5ms. We must wait at least this long for the 
    // first valid sample to be ready after waking up.
    k_msleep(45);

    // Step B: Take the reading
    double ax, ay, az;
    board_imu.read_acceleration(ax, ay, az);
    LOG_INF("IMU | X: %7.2f | Y: %7.2f | Z: %7.2f", ax, ay, az);

    // Step C: Put the chip immediately back to sleep!
    board_imu.sleep();

    // ---------------------------
    
    // Sleep for a while to save power (50ms is too fast for power cycling)
    k_msleep(500);
  }
}
...
/* ── Thread Declarations ──────────────────────────────
 *
 * Zephyr's K_THREAD_DEFINE macro does three things at compile time:
 *   1. Statically allocates the stack memory for the thread.
 *   2. Initializes the thread's internal tracking structures.
 *   3. Registers it to start automatically during system boot (since delay=0).
 *K_THREAD_DEFINE
 * Parameters:
 *   [Name]  [Stack Size]  [Entry Function] [Args x3] [Priority] [Options]
 * [Delay]
 */
...
K_THREAD_DEFINE(sensor_tid, 1024, sensor_thread_func, NULL, NULL, NULL, 7, 0,
                0);
